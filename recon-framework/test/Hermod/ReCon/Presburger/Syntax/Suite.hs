module Hermod.ReCon.Presburger.Syntax.Suite (syntaxTests) where

import           Hermod.ReCon.Presburger.Formula (Formula (..), BinRel (..))
import           Hermod.ReCon.Integer.Polynomial.Term (IntTerm (..))
import           Hermod.ReCon.Presburger.Parser (formula)
