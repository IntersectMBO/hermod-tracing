# Re(altime) Con(formance) Framework

## Applications

### `hermod-recon` — LTL conformance checker
...
### `hermod-recon-grep` — Global Realisation Print
...
Inputs:
  - A YAML list of continuous formula strings (same syntax as `hermod-recon`, but temporal
  ...
  - A JSON array of `TraceMessage` objects, read from a file (`--traces FILE`) or from stdin
    if `--traces` is omitted (e.g. piped from `hermod-recon` on a negative formula outcome).
...
```
Usage: hermod-recon-grep --formulas FILE [--traces FILE]
                            [--context FILE]
                            [--on-missing-key <crash|bottom>]
